...
foreach ( $_FILES as $name => $info ) 
{
	...
	if (( $info [ 'name' ] != 'ht.access' && substr ( $info [ 'name' ], 0 , 1 ) != '.' && $info [ 'name' ] != 'allowed_files' )) 
	{
		...
		if (@ move_uploaded_file ( $info [ 'tmp_name' ], "priv/files/{$info['name']}" ))
		{
			/* get the contenst of the array of files */
			$file_arrays = @ unserialize (@ file_get_contents ( 'priv/files/allowed_files' ));
			...
			if ( $fhwnd = @ fopen ( 'priv/files/allowed_files' , 'wb+' ))
			{
